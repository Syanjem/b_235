...
=======
 测试新分支（一旦你在 update/0917 上产生新提交，两者就会分叉）
...
1432 refactor: 更新 motorData 形参名
 ...
 adc 检测模式：
 timer0 输出关闭，但保持使能，保持产生更新事件。
 gpio 在对应口持续输出高/低电平。
 adc 触发模式。

 adc foc 模式：
 gpio 口关闭（直接初始化 timer0）
 timer0 开始输出
 adc 触发模式
...
如果，【电流与磁场反向，但是程序当成同向】
1.校准磁编的方向：校准方向的部分没问题，得出的方向与实际是一样的。
2.校准零点：校准零点输出A出（0°）的磁场A-BC+，输出磁场实际指向180°，测出的零点实际是【180°】
3.电流环：在理论情况下，有iq+反转，iq-正转。
由于计算所用的ea为实际ea+180°，所以还是iq+正转，iq-反转。
