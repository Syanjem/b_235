...
// encoder_spi
#define ONE_BY_ENCODER		(0.000061038881767686f)		//	1/16383.0f
...
//		APB2					120MHz（PWM 20 KHz）
#define TIMER0_PRESCALER		1
#define TIMER0_PERIOD			3000
//		占空比范围				[1200, 2700]
#define TIMER0_DEADTIME		 	36
#define TIMER0_PWM_MODE			TIMER_OC_MODE_PWM1	// PWM 模式1
#define TIMER0_CH0_CCR		 	1500
#define TIMER0_CH1_CCR			2000
#define TIMER0_CH2_CCR			2500
#define TIMER0_CH3_CCR			2990


#define M_PI			(3.14159265358979323846f)	// 圆周率
#define M_2PI			(6.28318530717958647692f)	// 2倍圆周率
#define M_PI_D_180		(0.0174532925199432957692369f)	// pi / 180
#define M_3PI_2			(4.71238898038469f)			// 3/2倍圆周率
#define _SQRT3			(1.7320508075688772935f)	// 3的平方根
#define _SQRT3_2 	(0.86602540378443864f)		// 3的平方根除以二
...
#define SQ(x)								((x) * (x))																						// 平方
#define NORM2_f(x, y)				(sqrtf(SQ(x) + SQ(y)))														// 二范数
#define DEG2RAD_f(deg)				((deg) * (float)(M_PI / 180.0f))				// 角度值转弧度值
#define RAD2DEG_f(rad)				((rad) * (float)(180.0f / M_PI))				// 弧度值转角度值
#define RPM2RADPS_f(rpm)			((rpm) * (float)((2.0f * M_PI) / 60.0f))					// 转每分值转角速度值
#define RADPS2RPM_f(rad_per_sec)	((rad_per_sec) * (float)(60.0f / (2.0f * M_PI)))			// 角速度值转转速值


#define ABS(a)							((a > 0.0f) ? (a) : (-a))			// 绝对值
#define min(a, b)						(((a) < (b)) ? (a) : (b))			// 取小值
#define max(a, b)						(((a) > (b)) ? (a) : (b))			// 取大值
#define CLAMP(x, lower, upper)		(min(upper, max(x, lower)))				// 限值范围
#define SIGN(x)								(((x) < 0.0f) ? -1.0f : 1.0f)		// 取符号
...
static inline void clarke_transform(float Ia, float Ib, float *Ialpha, float *Ibeta)
{
    *Ialpha = Ia;
    *Ibeta = (2 * Ib + Ia) * ONE_BY_SQRT3;
}
...
/* ============ FOC 标幺化基准值 (Per-Unit Base Values) ============ */
#define I_BASE 		33.0f	// ADC 双极性满量程: (3.3/2)V * 20A/V = 33A
#define U_BASE 		12.0f	// 母线电压基准, 24V/2=12V (相电压峰值基准)



/* 选额定相电流峰值作为 I_base，对应 id/iq=1.0pu 时为满载电流 */
//#define R_sense		1mΩ
//#define G				50
//#define ADC_Vref		3.3V
//#define offset		1.65V
...
/* 选额定机械角速度作为 ω_base，对应 speed=1.0pu 时为额定转速 */
#define W_BASE              (100.0f)   // [rad/s] ≈ 955 RPM，按你电机调
/* 基准磁链（导出量）：ψ_base = U_base / ω_base，用于反电势标幺化 */
#define PSI_BASE            (U_BASE / W_BASE)
